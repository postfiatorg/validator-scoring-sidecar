...
``response_parser.py``, ``unl_selector.py``, and ``score_formula.py`` must not
be imported at runtime: they contain foundation imports
(``scoring_service.config.settings``, ``scoring_service.services.prompt_builder``,
``scoring_service.services.response_parser``) that would fail in the sidecar
environment, so their adapted, runnable copies live in
``validator_scoring_sidecar.scoring.parser``,
``validator_scoring_sidecar.scoring.selector``, and
``validator_scoring_sidecar.scoring.formula`` instead.
