...
Vendored from foundation ``scoring_service/services/unl_selector.py``. Local
adaptations: ``cutoff``, ``max_size``, and ``min_gap`` are now required
keyword-only parameters passed from the round's execution manifest at
``code.selector.parameters`` (the foundation's ``settings`` fallback is
removed). See the package docstring in ``__init__.py`` for the refresh
procedure.
"""
...
def select_unl(
    ...
    *,
    ...
    Args:
        ...
        cutoff: Minimum score to qualify (from manifest
            ``code.selector.parameters.score_cutoff``).
        max_size: Maximum UNL size (from manifest
            ``code.selector.parameters.max_size``).
        min_gap: Minimum score margin for challenger displacement (from
            manifest ``code.selector.parameters.min_score_gap``).
