...
Vendored from foundation ``scoring_service/services/response_parser.py``.
The only local adaptation is defining ``ValidatorIdentityMap`` inline so the
module does not import from foundation code; see the package docstring in
...
def parse_response(
    ...
    Args:
        raw_text: Raw text content from the LLM endpoint.
        validator_id_map: Mapping of anonymous IDs to validator identities,
            built from the frozen ``inputs/validator_map.json`` file in the
            round's input package.

    Returns:
        ScoringResult with validated scores, or an incomplete result with
        error details.
    """
    errors: list[str] = []
