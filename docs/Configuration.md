...
| `POSTFIAT_SIDECAR_INFERENCE_TIMEOUT_SECONDS` | `180` | Bounds one scoring pass's inference work. On the direct transports (local SGLang, or a Modal deploy predating the job interface) it is the request's read timeout. On the Modal submit-and-poll transport it is the pass's polling budget — a fresh generation that outlives it is *not lost*: the pass records a resumable in-progress outcome (`reason: generation_in_progress`) and the next pass resumes the same server-side call, typically completing a normal round within a pass or two. When a round's commit window is close the budget is shortened to the time remaining, and scoring is skipped entirely if too little is left. Raise it to make single-pass completion likelier; the participate watchdog is derived from it, so the two stay consistent automatically. |
...
| `POSTFIAT_SIDECAR_MODAL_SCALEDOWN_MINUTES` | `5` | Idle minutes Modal keeps the GPU container — and its billing — alive after a request, before scaling to zero. Accepts 1 to 20 (Modal caps the window at 20 minutes). Production participation is one inference per weekly round, so a long window mostly bills an idle H100; the default keeps just enough warmth that a scoring pass retried on the next loop tick does not pay a fresh cold start. The window is infrastructure tuning, not part of the execution manifest, so changing it never affects scoring reproducibility or manifest compatibility. A changed value applies when the Modal app is next deployed: the participate loop redeploys automatically when the foundation pins a new runtime, or run `deploy-modal` once to apply it immediately. Modal-mode only; a local SGLang runtime is unaffected. |

| `HTTPS_PROXY`, `NO_PROXY` | unset | Standard proxy variables, honored natively by all of the sidecar's HTTP clients. Needed only when the egress path caps connection lifetimes (VPN, corporate firewall, NAT) and kills the long-lived inference request — see [Troubleshooting](Usage.md#troubleshooting). `HTTPS_PROXY` is process-wide (inference, scoring service, IPFS, PFTL RPC), so put every host that must stay direct — notably a local PFTL RPC — in `NO_PROXY`. |
...
| `HF_TOKEN` | Optional Hugging Face token (secret; read-only scope is enough) for authenticated model weight downloads on first deploy — the Hub throttles anonymous snapshot traffic. Modal mode reads it from `.env` and injects it into the GPU container as a runtime Modal Secret, never through the baked image environment. For local SGLang, `.env` is a no-op — export it in the GPU-host shell that runs `start-sglang` instead. Authentication only — the manifest-pinned revision and served weights are unchanged, and absent the variable downloads stay anonymous as before. See [Weight downloads and `HF_TOKEN`](Deployment.md#weight-downloads-and-hf_token). |
