...
| Scoring fails `endpoint_unreachable` with a "server disconnected" message after a *fixed* interval (for example always ~60s) | The egress path to the inference endpoint runs through a VPN, corporate firewall, or NAT that caps connection lifetime. Scoring holds one long-lived HTTPS request — Modal's long-poll redirect only fires around 150s — so the middlebox kills it first, every time. Each killed attempt still bills a full server-side generation. | Route the capped egress through a CONNECT proxy on a host outside the tunnel via the standard `HTTPS_PROXY` variable in `.env` — the sidecar's HTTP clients honor it natively. Note it is process-wide: it also proxies the scoring-service/IPFS client and the PFTL RPC, so list every host that must stay direct in `NO_PROXY` — above all a local or LAN PFTL RPC node, or participation breaks with `PFTL RPC … is not reachable`. |
