...
You need a host with Docker and Docker Compose — and nothing else. The default verify-only mode requires no validator at all: no validator keys, no wallet, no GPU. Anyone can run it as a zero-risk first step that independently checks the foundation's frozen inputs; a validator and its key file only become relevant if you later opt into on-chain participation. There is nothing to clone or build: the sidecar ships as published Docker images (`agtipft/validator-scoring-sidecar`), built and gate-checked by CI from the repository's environment branches. You only download the compose files and an environment template.
...
| Round `error_category` = `RUNTIME_UNAVAILABLE` | Two distinct conditions, told apart by `reason` in `error_details`: `endpoint_unreachable` means the network path to the inference endpoint failed (the failing URL and transport error are in the `message` detail), while `configuration` means the runtime setup is incomplete — no deployment record or missing credentials | For `endpoint_unreachable`, check the path from the container to the endpoint — see the connection-lifetime row below. For `configuration`, run `deploy-modal` / `start-sglang` (or set Modal credentials for zero-touch); for local SGLang confirm `POSTFIAT_SIDECAR_LOCAL_ENDPOINT_URL` is reachable from inside the container. |
| Scoring fails `endpoint_unreachable` with a "server disconnected" message after a *fixed* interval (for example always ~60s) | The egress path to the inference endpoint runs through a VPN, corporate firewall, or NAT that caps connection lifetime. Over the direct transport scoring holds one long-lived HTTPS request — Modal's long-poll redirect only fires around 150s — so the middlebox kills it first, every time. Each killed attempt still bills a full server-side generation. | This mode only affects Modal deployments that predate the submit-and-poll job interface: redeploy the app (`deploy-modal`, or wait for the loop's next automatic redeploy) and scoring switches to short polling calls that no lifetime cap can kill. Until then, route the capped egress through a CONNECT proxy on a host outside the tunnel via the standard `HTTPS_PROXY` variable in `.env` — the sidecar's HTTP clients honor it natively. Note it is process-wide: it also proxies the scoring-service/IPFS client and the PFTL RPC, so list every host that must stay direct in `NO_PROXY` — above all a local or LAN PFTL RPC node, or participation breaks with `PFTL RPC … is not reachable`. |
| Round `error_category` = `INFERENCE_TIMEOUT` | Three cases, told apart by `reason` in `error_details`: `generation_in_progress` means the submit-and-poll pass ran out of budget while the generation keeps running server-side; `round_deadline_elapsed` means too little of the commit window remained to attempt scoring; no marker means a direct-transport request truly timed out | `generation_in_progress` is not a fault — the call id is persisted and the next pass resumes the same generation, typically completing within a pass or two; raise `POSTFIAT_SIDECAR_INFERENCE_TIMEOUT_SECONDS` if you want single-pass completion. A true direct-transport timeout means the endpoint is overloaded or down (Modal cold starts are followed through their `303` long-poll redirect automatically, so a `303` is not the cause); on a slower self-hosted SGLang runtime raise the timeout (see [`Configuration.md`](Configuration.md)) — the commit-deadline cap still applies. |
