...
| Round `error_category` = `RUNTIME_UNAVAILABLE` | No deployment record, or the endpoint is unreachable | Run `deploy-modal` / `start-sglang` (or set Modal credentials for zero-touch); for local SGLang confirm `POSTFIAT_SIDECAR_LOCAL_ENDPOINT_URL` is reachable from inside the container. |
| Round `error_category` = `INFERENCE_TIMEOUT` | The model did not respond within the inference timeout | Modal cold starts are followed through their `303` long-poll redirect automatically, so a `303` is not the cause; a true timeout means the endpoint is overloaded or down. |
